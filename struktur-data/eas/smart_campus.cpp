...
void cetak_tree(Tree& tree, const string& nama, const string& prefix, bool is_last, bool is_root = false){
    if(is_root) cout << nama << endl;
    else cout << prefix << (is_last ? "|___ " : "|-- ") << nama << endl;
    auto it = tree.nodes.find(nama);
    if(it == tree.nodes.end() || it->second.empty()) return;
    string newPrefix = prefix;
    if(!is_root) newPrefix += (is_last ? "    " : "|   ");
...
// Mock Data
void mockdata(){
    akun_mahasiswa["hilmy052"] = {"5025251052", "Hilmy Fausta Pratama", "Teknik Informatika", 2, "hilmy123"};
    akun_mahasiswa["reza014"] = {"5022251014", "Reza Ferdian", "Teknik Elektro", 2, "reza123"};
    akun_mahasiswa["kenzy063"] = {"5010251063", "Kenzy Viriyananda", "Teknik Industri", 2, "kenzy123"};
    ...
    struktur_organisasi.nodes["WR Akademik"].push_back("Fakultas Teknologi Elektro dan Informatika Cerdas");
    struktur_organisasi.nodes["WR Akademik"].push_back("Fakultas Sains dan Analitika Dat");
    struktur_organisasi.nodes["WR Akademik"].push_back("Fakultas Teknologi Industri dan Rekayasa Sistem");
    struktur_organisasi.nodes["WR Akademik"].push_back("Fakultas Teknik Sipil, Perencanaan, dan Kebumian");
    struktur_organisasi.nodes["WR Akademik"].push_back("Fakultas Teknologi Kelautan");
    struktur_organisasi.nodes["WR Akademik"].push_back("Fakultas Desain Kreatif dan Bisnis Digital");
    struktur_organisasi.nodes["WR Akademik"].push_back("Fakultas Vokasi");
    struktur_organisasi.nodes["WR Akademik"].push_back("Fakultas Kedokteran dan Kesehatan");
}
...
void antrian(){
    ...
    while(true){
        ...
        if(opsi == 4){
            ...
            if(!antrian_konsultasi.empty()) tampil_konsultasi(antrian_konsultasi.front());
...
void operasi_krs(){
    ...
    while(true){
        ...
        if(opsi == 4) return;
            ...
            if(!stack_krs.empty()) cout << "Pilihan terbaru: " << stack_krs.top() << endl << endl;
